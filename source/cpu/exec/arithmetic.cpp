...
namespace z86
{
	...
	constexpr inline void set_OPZSA(CPU& cpu, Value& a, Value& b, Value& ret, bool sign, bool same_sign)
	{
	...
	constexpr inline void set_PZS(CPU& cpu, Value& a, Value& b, Value& ret)
	{
		cpu.flags().setPF(ret.parity());
		cpu.flags().setZF(ret.zero());
		cpu.flags().setSF(ret.sign());
